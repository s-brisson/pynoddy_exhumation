...
def generateSubFile(ndraws,interval,resolution,folder):
    ...
    JobSubFile_Groupable = f"/home/ho640525/projects/Exhumation/ClusterSubmission/Sub/Groupable_{timestamp}_NJOBS{n_jobs}.sh"
    JobSubFile_Modulus = f"/home/ho640525/projects/Exhumation/ClusterSubmission/Sub/Modulus_{timestamp}_NJOBS1.sh"
    ...
    with open(JobSubFile_Groupable, 'w') as sout:
        ...
        sout.write(f"#SBATCH --output={this_job_dir}/goupable_output_%A_%a.txt\n")
        ...
        sout.write(f"/home/ho640525/projects/Exhumation/ClusterSubmission/execute_job.sh {N_SIMULATIONS_PER_JOB} {interval} {resolution} {folder}\n")
    ...
    if  n_job_modulus!= 0:
        with open(JobSubFile_Modulus, 'w') as sout:
            ...
            sout.write("#SBATCH --mem-per-cpu=4000MB\n")
            sout.write("### Declare the merged STDOUT/STDERR file\n")
            sout.write(f"#SBATCH --output={this_job_dir}/modulus_output_%A_%a.txt\n")
            sout.write(f"# {ndraws} jobs will run in this array at the same time\n")
            sout.write(f"#SBATCH --array=1-1\n")
            sout.write("# each job will see a different ${SLURM_ARRAY_TASK_ID}\n")
            sout.write("echo \'now processing task id:: \' ${SLURM_ARRAY_TASK_ID}\n")
            sout.write(f"/home/ho640525/projects/Exhumation/ClusterSubmission/execute_job.sh {n_job_modulus} {interval} {resolution} {folder}\n")
